...
/**
 * Implementation of SRTP for PrivateGSM
 * Note this is not a full implementation and provides no support for SRTCP
 */
public class SRTP {
    ...
    private byte[] txAuthResultArray;              // Used by Tx Authentication for final result
    ...
    private byte[] rxAuthResultArray;              // Used by Rx Authentication for final result
    private byte[] initVector;                     // Initialisation Vector for AES encryption
    
    private LongSortedVector replayWindow;   // Stores received sequence numbers to allow replay protection checks
     ...
     * @param   keyDefinitionRate    New key definition rate is 2^aKDR, and maximum is 64 (2^64)
     ...
     * @param   key    Master Key string
     * @return  true if successfully set, false otherwise (invalid key length)
     */
    public boolean setTxMasterKey(byte[] key) {
        boolean res = false;
        if ((key.length == MASTER_KEY_SIZE_32_BYTES) || (key.length == MASTER_KEY_SIZE_16_BYTES)) {
            txMasterKey = null;
            txMasterKey = platform.getUtils().copy(key);
            res = true;
            logDebug("setTxMasterKey " + platform.getUtils().byteToHexString(txMasterKey));
        } else {
    ...
    public boolean setRxMasterKey(byte[] key) {
        ...
        if ((key.length == MASTER_KEY_SIZE_32_BYTES) || (key.length == MASTER_KEY_SIZE_16_BYTES)) {
            ...
            res = true;
            logDebug("setRxMasterKey " + platform.getUtils().byteToHexString(rxMasterKey));
        } else {
    ...
    public boolean setRxMasterSalt(byte[] salt) {
        ...
        if (salt.length == MASTER_SALT_SIZE_BYTES) {
            ...
            res = true;
            logDebug("setRxMasterSalt " + platform.getUtils().byteToHexString(rxMasterSalt));
        } else {
            logError("Wrong length for iRxMasterSalt: "+salt.length+" expected "+MASTER_SALT_SIZE_BYTES+" bytes");
        }
        return res;
    }
    ...
    public boolean setTxMasterSalt(byte[] salt) {
        ...
        if (salt.length == MASTER_SALT_SIZE_BYTES) {
            ...
            res = true;
            logDebug("setTxMasterSalt " + platform.getUtils().byteToHexString(txMasterSalt));
        } else {
            logError("Wrong length for iTxMasterSalt: "+salt.length+" expected "+MASTER_SALT_SIZE_BYTES+" bytes");
        }
        return res;
    }
    ...
    /**
     * Sets the 1st RTP Sequence number to be used in the next session. Needed by SRTP session key generation.
     * If not set, a sequence number of 1 is used.
     * Setting the sequence number after a session is started has no effect.
     * @param   seq    sequence number of 1st rtp packet
     */
    ...
    /**
     * Starts a new SRTP Session, using the preset master key and salt to generate the session keys
     * @return  error code
     */
    public int startNewSession() {
        if (txSessEncKey != null) return SESSION_ERROR_ALREADY_ACTIVE;
        if ((txMasterSalt == null) || (rxMasterSalt == null)) return SESSION_ERROR_MASTER_SALT_UDNEFINED;
        if ((txMasterKey == null) || (rxMasterKey == null)) return SESSION_ERROR_MASTER_SALT_UDNEFINED;
        ...
        try {
            ...
            txEncryptorSuite = platform.getCrypto().createEncryptorSuite(txSessEncKey, initVector);
    ...
    /**
     * Protects an RTP Packet by encrypting payload and adds any additional SRTP trailer information
     * @param   packet RTP Packet to be protected
     * @return  New RTP Packet with encrypted payload, or null if an error occurred
     *          If encryption is disabled, returned packet is identical to supplied packet
     */
    public RtpPacket protect(RtpPacket packet) {
        ...
        if (!transformPayload(packet, rollOverCounter, seqNum, txSessSaltKey, true)) {
            log("protect() transformPayload error, encryption failed");
            return null;
        }
        
        // Add authentication which is over whole rtp packet concatenated with 48 bit ROC
        byte[] auth = null;
        try {
    ...
    private byte[] getAuthentication(RtpPacket packet, long roc, boolean isTransmitting) throws CryptoException
    {
        // returns an authentication array which may be added to an RTP payload (when sending)
    ...
    private boolean transformPayload(RtpPacket rtpPacket, long roc, int aSeqNum,
                                     byte[] sessSaltKey, boolean isTransmitting) {
        // Returns false if error occurs
        // Uses passed parameters aROC & aSeqNum to calculate packet index
        // as, when unprotecting, its an index estimate (packets may have been missed)
        ...
        if (VERBOSE) {
            log("transformPayload, after init, IV = " + platform.getUtils().byteToHexString(IV));
        }
        ...
        if (iter > 0) {
            for (int i = 0; i < iter; ++i) {
                ...
                // IV encrypted, now XOR with payload segment & append to new payload
    ...
    private byte[] encryptIV(byte[] iv, boolean isTransmitting) throws CryptoException {
        if (isTransmitting) {
    ...
    private void initialiseIV(byte[] iv, long ssrc, int seq, long roc, byte[] aSessSaltKey) {
        ...
        // Now xor with SSRC*2^64 - aSsrc is a long only to make it unsigned, only 4 LSB of interest
        for (int i = 4; i < 8; ++i) {
        ...
        long tempROC = roc >>> 16; // Only a long to make it unsigned, only 4 bytes of interest
        for (int i = 11; i >= 8; --i) {
     ...
     * @param   packet RTP Packet to be unprotected
     * @return  error code,  0 = success
     */
    public int unprotect(RtpPacket packet) {
        if (txSessAuthKey == null) {
            // Only the tx session key is set at session start, rx is done when 1st packet received
        ...
        // of XORing payload with an encrypted IV to obtain original payload data
        ...
        // Payload now unprotected. Update the latest seq & ROC ready for next packet
        if (v == rxRoc) {
            if (seq > rxSeq) {
                rxSeq = seq;
            }
    ...
    private boolean isReplayedPacket(long aSeq) {
        ...
        if (found < 0) {
            ...
            if (aSeq > curSeq) {
                ...
                // Readjust window contents to delete those older than window size
                long oldestSeq = aSeq - SRTP_WINDOW_SIZE;
                int numToRemove = 0;
                for (int i = 0; i < numElements; i++) {
                    Long l = (Long)(replayWindow.getAt(i));
                    long seq = l.longValue();
                    if (seq < oldestSeq) {
                        numToRemove++;
                    } else {
                        // Since its a sorted array, once we've seen a sequence number inside
    ...
    private boolean txSessionKeyDerivation() {
		 ...
		 *   alignment).
		 *       
		 *  Note that test data provided in rfc3711 suggests that key_id is a 7 byte value
		 *  whereas the above suggests a 9 byte value. i.e. for the calculation of x,
		 *  key_id is right aligned with master_salt*16
		 */
         
        boolean res = true;
        /*
         * Following calc not needed as for initial key derivation, index is 0
         * long index = iROC + iFirstRtpSeq;
         * log("txSessionKeyDerivation()");
         * log("+++ iROC = " + iROC);
         * log("+++ iFirstRtpSeq = " + iFirstRtpSeq);
         * log("+++ index = " + index);
         * long r = index >>> iKDR;
         * log("+++ r = " + r);
		 *
         * Need to produce 3 keys (session encryption, session authentication and session salt)
         * All based on same value of r, above, but with labels of 0x00, 0x01 & 0x02
         */
        byte label = 0x00;
        if(VERBOSE) {
        ...
        while (label <= 0x02) {
            ...
            if (label == 0) {
                ...
                if(VERBOSE) {
                	logDebug("+++ enc key = " + platform.getUtils().byteToHexString(txSessEncKey));
                }
            } else if (label == 1) {
                ...
                if(VERBOSE) {
                	logDebug("+++ auth key = " + platform.getUtils().byteToHexString(txSessAuthKey));
                }
            } else {
                ...
                if(VERBOSE) {
                	logDebug("+++ salt key = " + platform.getUtils().byteToHexString(txSessSaltKey));
                }
            }
            label ++;
        }
    ...
    private boolean rxSessionKeyDerivation() {
        ...
        while (label <= 0x02) {
            ...
            if (label == 0) {
                ...
                if (rxSessEncKey == null) {
                	...
                	if(VERBOSE) {
                		logDebug("+++ enc key = " + platform.getUtils().byteToHexString(rxSessEncKey));
                	}
                }
            } else if (label == 1) {
                ...
                if (rxSessAuthKey == null) {
                	...
                	if(VERBOSE) {
                		logDebug("+++ auth key = " + platform.getUtils().byteToHexString(rxSessAuthKey));
                	}
                }
            } else {
                ...
                if (rxSessSaltKey == null) {
                	...
                	if(VERBOSE) {
                		logDebug("+++ salt key = " + platform.getUtils().byteToHexString(rxSessSaltKey));
                	}
                }
            }
            label ++;
        ...
        if (res) {
            ...
            try {
                rxEncryptorSuite = platform.getCrypto().createEncryptorSuite(rxSessEncKey, initVector);
                rxHMAC = platform.getCrypto().createHMACSHA1(rxSessAuthKey);
            } catch (Throwable e) {
                logError("rxSessionKeyDerivation failed to create Tx encryptor EX: "+e);
                e.printStackTrace();
                res = false;
            }
        }
    ...
    private byte[] prf_128(byte[] key, byte[] x, int outLen) {
        ...
        // see rfc3711, 4.3.3
        //
        // Assumes here that aX is 14 bytes long, which is safe in this implementation
        // because we operate with a fixed 112 bit master salt (as indicated in zrtp spec)
        // For a more general srtp impl, would need to check size and pad accordingly
        //
        byte[] IV = new byte[16]; 
        platform.getUtils().zero(IV);
        ...
        try {
            ...
            EncryptorSuite encSuite = platform.getCrypto().createEncryptorSuite(key, initVector);
            ...
            for (int i = 0; i < maxLoops; ++i) {
                ...
                if (VERBOSE) {
                    log("prf_128 IV = " + platform.getUtils().byteToHexString(IV));
                    log("prf_128 loop count = " + i);
                    log("prf_128 outBlock = " + platform.getUtils().byteToHexString(outBlock));
                }
                baos.write(outBlock);
                incrementIV(IV);
            }
    ...
    private void logDebug(String msg) {
    	if(platform.isDebugVersion()) log(msg);
    }
...
//
    ...
    public boolean testEncryption() {
        ...
        boolean ret = true;
        rollOverCounter = 0L;
        byte[] sessKey = {(byte)0x2B,(byte)0x7E,(byte)0x15,(byte)0x16,(byte)0x28,(byte)0xAE,(byte)0xD2,(byte)0xA6,
                          (byte)0xAB,(byte)0xF7,(byte)0x15,(byte)0x88,(byte)0x09,(byte)0xCF,(byte)0x4F,(byte)0x3C};
        byte[] sessSalt = {(byte)0xF0,(byte)0xF1,(byte)0xF2,(byte)0xF3,(byte)0xF4,(byte)0xF5,(byte)0xF6,(byte)0xF7,
        ...
        try {
        	...
        	EncryptorSuite encSuite = platform.getCrypto().createEncryptorSuite(sessKey, initVector);
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            for (int i = 0; i < 3; ++i) {
    ...
    public boolean testReplayWindow() {
        ...
        if (TestSettings.TEST) {
            ...
            if (isReplayedPacket(seq)) {
                log("testReplayWindow incorrectly reported repeated packet id " + seq);
                ret = false;
            }
            Long l = (Long)replayWindow.getAt(0);
            long first = l.longValue();
            if (first != 5) {
                log ("testReplayWindow after deleting elements, incorrect first element " + first);
                ret = false;
            }
            ...
            l = (Long)replayWindow.getAt(0);
            first = l.longValue();
            if (first != 5) {
                log ("testReplayWindow after adding in-window element, incorrect first element " + first);
                ret = false;
            }
        }
        return ret;
