...
//import com.privategsm.localisation.Localisation;

/**
 * Handles ZRTP protocol and key generation/exchange
 */
public class ZRTP {
    /* Flag whether to use s1 instead of rs1 as the new rs2 when updating the shared secret cache.
     * This deviation from the ZRTP spec, section 4.6.1., prevents a rare situation when two missed
     * updates on one client will push the other client out of sync and cause a cache mismatch.
     * At first look, this should not compromise security but a deeper analysis was not performed. */
    ...
    // The optional responder timeout between Commit and Confirm2, see ZRTP spec, section 6
    private final static int RESPONDER_TIMEOUT = 40000; // 20s (more than the suggested 10s)
    ...
    private boolean DH_MODE_EC_USE_256 = TestSettings.KEY_TYPE_EC25; // ec25 or ec38
    ...
    private long lastPacketArrival; // last ZRTP packet arrival, for responder timeout
    ...
    //Accurately saved sent and received messages needed for shared secret calculation
    ...
    //Optional: Hash of the Hello message to be received. This hash is sent by the other
    //end as part of the SDP for further verification. If not received we wont check the Hello message
    ...
    public ZRTP(Platform platform) {
        ...
        /* Force client to be initiator or responder dependending
         * on testing settigs
         */
        /*
        ...
        /*
        if(forceToBeInitiator && forceToBeResponder) {
        	logError("TEST SETTINGS REQUIRE TO BE INITIATOR AND RESPONDER AT THE SAME TIME!!!");
        	forceToBeInitiator = false;
        	forceToBeResponder = false;
        }
        */
    }
     ...
     *
     * @param version ZRTP version of the hash
     * @param helloHash Hello hash received as part of SDP in SIP
     */
    public void setSdpHelloHash(String version, String helloHash) {
        if (!version.startsWith(VERSION_PREFIX)) {
            logWarning("Different version number: '" + version + "' Ours: '" + VERSION_PREFIX + "' (" + VERSION + ")");
        }
        sdpHelloHashReceived = helloHash;
    }
    
    /**
     * Sets the phone number of other party for use in addressbook lookup during verification
     * @param number Phone number of the other party
     */
    ...
    /**
     * Retrieves the trust status of the connection reported by the remote client.
     * @return true iff the connection is trusted.
     */
    ...
    /**
     * Notifies the ZRTP protocol that the user has chosen to trust the connection.
     * SAS verified flags is set to true and shared secrets are stored in the cache
     * (they should already be in the cache unless they were removed by untrust() or
     * delayed because of cache mismatch).
     */
    public void trust() {
    ...
    /**
     * Notifies the ZRTP protocol that the user has revoked their trust in the connection.
     * SAS verified flags is set to false and shared secrets with the remote end are removed
     * from the cache.
     */
    public void untrust() {
    ...
    private void logBuffer(String str, byte[] buf) {
        ...
        if(platform.isDebugVersion()) platform.getLogger().log("      " + platform.getUtils().byteToHexString(buf, 0, buf.length)+ " len:"+buf.length);
    }

    private void logBuffer(String str, byte[] buf, int offset, int len) {
    	platform.getLogger().log("ZRTP: " + str);
    	platform.getLogger().log("      " + platform.getUtils().byteToHexString(buf, offset, len)+ " len:"+len);
    }
    ...
    private void logZrtpMessage(String str, byte[] buf, int offset, int len) {
    	...
    	for (; i < len/4; i++) {
    		sb.append("      ");
    		sb.append(platform.getUtils().byteToHexString(buf, offset+i*4, 4));
    		sb.append("\n");
        }
    	sb.append("      " + platform.getUtils().byteToHexString(buf, offset+i*4, len - i*4));
    ...
    private void runSession() {
        if (!started) {
            ...
            try {
                ...
                logError("Exception sending initial Hello message: " + e.toString());
                e.printStackTrace();
                completed = true;
            }
            while (!completed) {
    ...
    private void endSession() {
        // For trust() and untrust() to work after the session has terminated we must keep
        // iCache, iNewRS, iKeepRS2, iCacheExpiry, iConfirm1Timestamp and iDelayedCacheUpdate.
        // iFarEndZID is not necessary as the cache has the correct ZID already selected.
        retranTimer.cancel();
        retranTask = null;
        started = false;
        // iRtpSender = null; // keep RTP sender for responding to Confirm2 retransmissions
        completed = true;
        if(hashChain != null) hashChain.clear();
        initiator = false;
    ...
    private byte[] createMessageBase(byte[] msgType, int length)
        ...
        // Creates a base ZRTP message containing 2 byte header, 2 byte length & 8 byte message
        // Supplied length is the number of 4 byte words to indicate in the message length
        if ((msgType.length != 8) || (length < 3)) {
            logError("createMessage(), invalid parameter, string " + msgType + ", length " + length);
    ...
    private void raiseDenialOfServiceWarning(String warning) {
        if ((securityWarningFlags & 1) == 0) {
            // TODO: should we log all warnings? we're logging only the first one for now
    ...
    private synchronized void retranTimerExpired(RetranTimerTask task) {
        ...
        try {
            ...
            if (state == ZRTP_STATE_SENDING_HELLO) {
                // Not had a HelloACK yet, so send another (unless we've reached the limit)
                ...
                if (retranCount < T2_MAX_RETRANSMISSIONS) {
                    ...
                    if(platform.getLogger().isEnabled()){
                    	logString("ZRTP initiator retransmission #"+retranCount+" in state "+getStateText());
                    }
                    switch (state) {
    ...
    private byte[] createHelloMsg() throws IOException {
        ...
        baos.write(VERSION.getBytes());
        String clientID = new String("KhamsaPrivateGSM"); // Must be 16 bytes long
        baos.write(clientID.getBytes("US-ASCII"));
        baos.write(hashChain.H3);
        byte[] zid = localZID;
        if(TestSettings.TEST && TestSettings.TEST_ZRTP_ZID_COLLISION && farEndZID != null) {
            // do not start sending Hello, but wait other peer's hello
            // in order to answer with the same ZID and trigger the ZID collision
            zid = farEndZID;
        }
        baos.write(zid);
        String hashes = "";
    ...
    private synchronized void sendHello() throws IOException {
        ...
        if(TestSettings.TEST && TestSettings.TEST_ZRTP_ZID_COLLISION && farEndZID == null) {
            // do not start sending Hello, but wait other peer's hello
            // in order to answer with the same ZID and trigger the ZID collision
        	platform.getLogger().log("Waiting sending HELLO to simulate ZID collision");
            sendPing();
        } else {
            if (txHelloMsg == null) {
     ...
     * @param offset Offset of the data for the hash
     * @param len Length of msg to be considered for calculating the hash
    ...
    private String calculateSHA256Hash(byte[] msg, int offset, int len) {
        ...
        // prepare space for hexadecimal representation, store the diggest in the second half and then convert
    ...
    private byte[] createSHA256HMAC(byte[] data, int offset, int length, byte[] aKey) {
    	return platform.getCrypto().calculateSHA256HMAC(data, offset, length, aKey);
    }
    	
    private byte[] createSHAHMAC(byte[] data, int offset, int length, byte[] aKey) {
	    return HashType.SHA384.equals(hashMode)
        ? platform.getCrypto().calculateSHA384HMAC(data, offset, length, aKey)
        : platform.getCrypto().calculateSHA256HMAC(data, offset, length, aKey);
    }

    private Digest createDigest(HashType hashType) {
	    return HashType.SHA384.equals(hashType)
	         ? (Digest)platform.getCrypto().createDigestSHA384()
	         : (Digest)platform.getCrypto().createDigestSHA256();
    }
    ...
    /**
     * @return SHA256 Hash of the Hello message to be included in the SDP
     * See: draft-zimmermann-avt-zrtp-17 Sections: 4.1, 8.1 & 5.1.2.1
     */
     ...
     *
     * @param helloMsg Contents of the Hello message received
    ...
    private boolean verifyHelloMessage(byte[] helloMsg) {
        if (sdpHelloHashReceived != null) {
            ...
            boolean hashesMatched = hash.toUpperCase().equals(sdpHelloHashReceived.toUpperCase());
            if(platform.getLogger().isEnabled()) {
	            if (hashesMatched) {
	                logString("verifyHelloMessage() Hello hash verified OK.");
	            } else {
	                logString("verifyHelloMessage() Hello hash does NOT match  hash= '" + hash + "' expected (SDP) = '" + sdpHelloHashReceived + "'");
	            }
            }
    ...
    private synchronized void sendHelloACK() {
        ...
        if(forceToBeInitiator && iState != ZRTP_STATE_COMMIT_SENT) {
        	// nothing to do. Do not send COMMIT message, so the other
        	// peer will wait before sending COMMITT and we will be Initiator
    ...
    private synchronized void sendCommit() throws IOException {
    	...
    	/*
    	if(forceToBeInitiator) {
    		// send an HELLO_ACK, to make the other peer state machine progress
    		logString("Instance forced to be initiator. Sending first HELLO_ACK");
    		iRtpSender.sendZrtpPacket(mMsgHelloACK);
    		forceToBeInitiator = false;
    	}
    	*/
        if (commitMsg == null) {
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            // Commit always has length 29 words in DH mode (which we always use)
    ...
    private void scheduleTimerResponderTimeout() {
        ...
        // reusing retransmision timer as responder never really needs to retransmit
        retranTask = new RetranTimerTask();
    ...
    private byte[] createHvi() {
        ...
        try {
            ...
            logError("Error while creating DHPart2 when calculating HVI EX: "+ex);
            ex.printStackTrace();
        }
    ...
    private byte[] createRemoteHvi(byte[] dhPart2Data, int offset, int len) {
        // Create the hvi (hash value initiator, see zrtp spec 4.4.1.1) that the remote
        // client should have calculated. It's checked against the hvi received in Commit.
        Digest digest = createDigest(hashMode);
    ...
    private void writeSharedSecretID(byte[] dhPart, int off, byte[] msg, byte[] secret) {
        if(secret == null) {
    ...
    private byte[] createDHPartX(byte[] msgType) throws IllegalArgumentException, ZrtpException {
        ...
        if (cache.getCurrentNumber() != null) {
        	...
        	if(!platform.getAddressBook().matchingNumbers(cache.getCurrentNumber(), phoneNumber)) {
                 ...
                 * 1) my contact change legitimately his own phone number.
                 * 2) someone stolen my device and try to use it with his own SIM, eg: because I blocked my own
                 * 
                 * To avoid the dangerouse second one, we raise a warning/hangup call
                 */
            ...
            if (cache.isNewZidForTrustedUser(phoneNumber)) {
                /* It happens in two scenarios
                 * 1) my contact reinstalled pgsm, creating a new ZID
                 * 2) a MITM call me using his own ZID and my contact Caller-ID. This could be dangerous 
                 *    if, knowing my contact (signaled in "incoming secure call popup") is trusted,
                 *    I do not pay attention to phone display showing the untrusted status due to MITM presence 
                 *    
                 * To avoid the dangerouse second one, we raise a warning/hangup call
                 */
        ...
        byte[] msg = (msgType == MSG_TYPE_DHPART2) ? mInitiatorBytes : mResponderBytes;
    ...
    private byte[] calculateS1() {
        // pre: iCache.selectEntry(iFarEndZID); has been called (in createDHPartX)
        byte[] rs1 = cache.getRetainedSecret1();
        byte[] rs2 = cache.getRetainedSecret2();
        if(initiator) {
            byte[] rs1mac = (rs1 == null) ? null : createSHAHMAC(mResponderBytes, 0, mResponderBytes.length, rs1);
            if(rs1mac != null && (platform.getUtils().equals(msgDhPart1RX, 44, rs1mac, 0, 8) || platform.getUtils().equals(msgDhPart1RX, 52, rs1mac, 0, 8))) {
                if(platform.getLogger().isEnabled()) {
                	logString("calculateS1: initiator - rs1; "+platform.getUtils().equals(msgDhPart1RX, 44, rs1mac, 0, 8));
                }
                return rs1;
            }
            byte[] rs2mac = (rs2 == null) ? null : createSHAHMAC(mResponderBytes, 0, mResponderBytes.length, rs2);
            if(rs2mac != null && (platform.getUtils().equals(msgDhPart1RX, 44, rs2mac, 0, 8) || platform.getUtils().equals(msgDhPart1RX, 52, rs2mac, 0, 8))) {
                if(platform.getLogger().isEnabled()) {
                	logString("calculateS1: initiator - rs2; "+platform.getUtils().equals(msgDhPart1RX, 44, rs2mac, 0, 8));
                }
                return rs2;
            }
        } else {
            byte[] rs1mac = (rs1 == null) ? null : createSHAHMAC(mInitiatorBytes, 0, mInitiatorBytes.length, rs1);
            if(rs1mac != null && platform.getUtils().equals(msgDhPart2RX, 44, rs1mac, 0, 8)) {
                if(platform.getLogger().isEnabled()) {
                	logString("calculateS1: responder - rs1 (a)");
                }
                return rs1;
            }
            byte[] rs2mac = (rs2 == null) ? null : createSHAHMAC(mInitiatorBytes, 0, mInitiatorBytes.length, rs2);
            if(rs2mac != null && platform.getUtils().equals(msgDhPart2RX, 44, rs2mac, 0, 8)) {
                if(platform.getLogger().isEnabled()) {
                	logString("calculateS1: responder - rs2 (b)");
                }
                return rs2;
            }
            if(rs1mac != null && platform.getUtils().equals(msgDhPart2RX, 52, rs1mac, 0, 8)) {
                if(platform.getLogger().isEnabled()) {
                	logString("calculateS1: responder - rs1 (c)");
                }
                return rs1;
            }
            if(rs2mac != null && platform.getUtils().equals(msgDhPart2RX, 52, rs2mac, 0, 8)) {
                if(platform.getLogger().isEnabled()) {
    ...
    private void calculateSharedKeys() throws IOException {
        // Check if we have a shared secret to be used as s1, see ZRTP spec, 4.3.
        ...
        kdfContext = new byte[24 + totalHash.length]; // ZIDi and ZIDr are always 12 bytes each
        if (initiator) {
        ...
        // Not actually a counter - just using same naming convention as in zrtp spec
        ...
        byte[] nullLength = {(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00};
        if (s1 == null) {
     ...
     *
     * @param ki S0
     * @param label Text string used in key/salt generation
     * @param context KDF_Context
     * @param l Length of the key/salt in bits
     * @return Key/salt with length of aL bits. If aL is smaller than 256, the
     * 32 bits result is truncated and aL leftmost bits are returned as specified
     * in section 4.5.3.
     * @throws IOException
     */
    private byte[] getKeyFromKDF(byte[] ki, String label, byte[] context, int l) throws IOException {
        int bytesCount = l >>> 3;
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        byte[] counter = new byte[]{(byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x01};
        baos.write(counter);
    ...
    private byte[] createConfirmMsg(boolean confirm1) throws CryptoException, IOException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        ...
        byte[] zrtpKey = getKeyFromKDF(s0, initiator ? "Initiator ZRTP key" : "Responder ZRTP key", kdfContext, 256); // AES key length. TODO
        byte[] hmacKey = getKeyFromKDF(s0, initiator ? "Initiator HMAC key" : "Responder HMAC key", kdfContext, dhMode.hash.getLength());
        byte[] plainBytes = baos.toByteArray();
        if(platform.getLogger().isEnabled()) {
        	logBuffer("Confirm plainBytes: ", plainBytes);
        }
        byte[] cipherBytes = platform.getCrypto().aesEncrypt(plainBytes, zrtpKey, cfbInitVector);
    ...
    private synchronized void sendConfirm2() throws IOException, CryptoException {
        ...
        if(msgConfirm2TX == null) {
            ...
            s0 = null; // we're done with s0, clear as required by ZRTP spec, section 4.6.1
        }
        sendZrtpPacket(msgConfirm2TX);
    ...
    private synchronized void sendConf2ACK() {
        ...
        boolean success = listener.keyExchangeCompleted(txMasterKey, txMasterSalt, rxMasterKey, rxMasterSalt, seqNum);
    ...
    public void handleIncomingMessage(byte[] data, int offset, int len) {
        ...
        if (!started) {
            logBuffer("handleIncomingMessage(), received message when not started", data, offset, len);
            return;
        }
        if (len < 12) {
            // Too short, must have at least 2 byte header (0x505A), 2 byte length indicator
            // and 8 byte message type
            logBuffer("handleIncomingMessage(), received message too short", data, offset, len);
            return;
        }
        if ((data[offset+0] != 0x50) || (data[offset+1] != 0x5A)) {
            logBuffer("handleIncomingMessage(), received invalid message header 0x"+Integer.toHexString((data[offset+1] << 8) + data[offset+0]), data, offset, len);
            return;
        }
        //Length in message is number of 4 byte words
        int msgLen = (data[offset+2] << 8) + data[offset+3];
        if (len != msgLen * 4) {
            logBufferWarning("handleIncomingMessage(), received message with wrong length defined in header as "+msgLen+
                      " words"+" ("+(msgLen*4)+" bytes) but have an array of "+len+" bytes", data, offset, len);
            return;
        }
        ...
        try {
            ...
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_HELLOACK, 0, 8)) {
                doHelloACK(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_COMMIT, 0, 8)) {
                doCommit(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_DHPART1, 0, 8)) {
                doDHPart1(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_DHPART2, 0, 8)) {
                doDHPart2(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_CONFIRM1, 0, 8)) {
                doConfirm1(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_CONFIRM2, 0, 8)) {
                doConfirm2(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_CONF2ACK, 0, 8)) {
                doConf2ACK(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_ERROR, 0, 8)) {
                doError(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_ERRORACK, 0, 8)) {
                doErrorACK(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_GOCLEAR, 0, 8)) {
                doGoClear(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_CLEARACK, 0, 8)) {
                doClearACK(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_SASRELAY, 0, 8)) {
                doSASrelay(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_RELAYACK, 0, 8)) {
                doRelayACK(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_PING, 0, 8)) {
                doPing(data, offset, len);
            } else if (platform.getUtils().equals(data, mtOff, MSG_TYPE_PINGACK, 0, 8)) {
                doPingACK(data, offset, len);
            } else {
                String msgType = new String(data, mtOff, 8);
                logWarning("handleIncomingMessage() message with invalid type = '"+msgType+"'");
                logBuffer("handleIncomingMessage(), message with invalid type", data, offset, len);
            }
        } catch (Throwable e) {
    ...
    private void doHello(byte[] data, int offset, int len) throws IOException {
        /* Format of hello is
         *      Byte 00 - 11    Header & message type
         *           12 - 15    Protocol version
         *           16 - 31    Client Identifier
         *           32 - 63    Hash image H3
         *           64 - 75    ZID
         *           76 - 79    Algorithm list header
         *           80 onwards Algorithm list
         *           */
        ...
        if (rxHelloMsg != null) {
            if (rxHelloMsg.length != len || !platform.getUtils().equals(rxHelloMsg, 0, data, offset, len)) {
            ...
            //If the received Hello msg doesn't have the same hash as sent in the SDP
            ...
            if (!checkVersion((char)aMsg[12], (char)aMsg[14])) {
                logWarning("doHello Ignoring Hello msg, version check failed! version major='"+((char)aMsg[12])+"' mid='"+((char)aMsg[13])+"' minor='"+((char)aMsg[14])+"'");
                return;
            }
            ...
            for (int i = 0; i < hashCount; i++) {
                // Only need to check for SHA-384 as, if its not there, we'll always use SHA-256
            	if(DH_MODE_EC_USE_256 && TestSettings.KEY_TYPE_EC25) {
                    if (platform.getUtils().equals(HashType.SHA256.getType(), 0, aMsg, hashPos + i*4, 4)) {
                        hashMode = HashType.SHA256;
                    }
            	} else if(!DH_MODE_EC_USE_256 && TestSettings.KEY_TYPE_EC38) {
            		if (platform.getUtils().equals(HashType.SHA384.getType(), 0, aMsg, hashPos + i*4, 4)) {
            			hashMode = HashType.SHA384;
            		}
                }
                if(platform.isDebugVersion()) {
                	logString("HELLO MSG - HASH: " + new String(aMsg, hashPos + i*4, 4));
                }
            }
            ...
            for (int i = 0; i < keyCount; i++) {
                if(DH_MODE_EC_USE_256 && TestSettings.KEY_TYPE_EC25) {
                    if (platform.getUtils().equals(KeyAgreementType.ECDH256.getType(), 0, aMsg, keyPos + i*4, 4)) {
                        dhMode = KeyAgreementType.ECDH256;
                    }                	
                } else if(!DH_MODE_EC_USE_256 && TestSettings.KEY_TYPE_EC38) {
	            	if (platform.getUtils().equals(KeyAgreementType.ECDH384.getType(), 0, aMsg, keyPos + i*4, 4)) {
	                    dhMode = KeyAgreementType.ECDH384;
	                }
                }
	            if(platform.isDebugVersion()) {
                	logString("HELLO MSG - KEY: " + new String(aMsg, keyPos + i*4, 4));
                }
            }
            dhSuite.setAlgorithm(dhMode);
            ...
            for (int i = 0; i < sasCount; i++) {
                // Only need to check for B256 as, if its not there, we'll always use B32
                if (platform.getUtils().equals(SasType.B256.getType(), 0, aMsg, sasPos + i*4, 4)) {
                    sasMode = SasType.B256;
                }
                if(platform.isDebugVersion()) {
                	logString("HELLO MSG - SAS: " + new String(aMsg, sasPos + i*4, 4));
                }
            }
            try {
                if (dhPart2Msg == null) {
    ...
    private synchronized void doHelloACK(byte[] data, int offset, int len) throws IOException {
        if(platform.getLogger().isEnabled()) {
        ...
        if (state == ZRTP_STATE_SENDING_HELLO) {
            ...
            if (rxHelloMsg != null) {
                ...
                if(forceToBeResponder) {
                	// nothing to do. Do not send COMMIT yet, and wait COMMIT from other
                	// peer so that we will be responder
                } else {
		        	initiator = true; // This might change if we receive a Commit and have to do contention
		            sendCommit();
    ...
    private synchronized void doCommit(byte[] data, int offset, int len) throws IOException {
        if (len != 116) {
            logWarning("doCommit() received invalid length message");
            return;
        }
        if(msgCommitRX != null) {
            if(msgCommitRX.length != len || !platform.getUtils().equals(msgCommitRX, 0, data, offset, len)) {
            ...
            if (!platform.getUtils().equals(helloMac, 0, rxHelloMsg, helloMacOffset, 8)) {
        ...
        if ((state == ZRTP_STATE_SENDING_HELLO) || 
            ...
            logString("Received Commit, acting as responder, iState: "+getStateText());
            commitMsg = extractData(data, offset, len);
            msgCommitRX = commitMsg;
            initiator = false;
            scheduleTimerResponderTimeout();
            state = ZRTP_STATE_GOT_COMMIT;
        } else if ((state == ZRTP_STATE_DHPART1_SENT) || (state == ZRTP_STATE_GOT_COMMIT)) {
            // We're already in Got_Commit or dhpart1_sent states, our DHPart1 didn't arrive
            // so, send it again
            if(platform.getLogger().isEnabled()) {
            	logString("Received Commit retransmission, iState: "+getStateText());
            }
            state = ZRTP_STATE_GOT_COMMIT;
        } else if (state == ZRTP_STATE_COMMIT_SENT) {
            // if we've already received Commit and we're in commit_sent state, we've already
            ...
            if(msgCommitRX == null) {
                ...
                // Since we only support DH mode, contention is based on comparison of hvi
                ...
                // hvi is treated as an unsigned integer for comparison (zrtp spec 4.2)
                for (int i = 0; i < 32; i++) {
                    ...
                    if (ourByte > theirByte) {
                        ...
                        if (commitMsg[63] == '1') {
                            // Already validated, so cipher string in bytes 60 - 63
                        ...
                        if (commitMsg[71] == '8') {
                            // Already validated, so key agreement type in bytes 68 - 71
    ...
    private boolean validateCommitMessage(byte[] data, int offset) {
        ...
        if (!platform.getUtils().equals(farEndZID, 0, data, offset+44, 12)) {
            ...
            logString("validateCommitMessage, Found invalid auth type - "+(new String(data, offset+64, 4)));
        		...
        		&& !platform.getUtils().equals(KeyAgreementType.ECDH256.getType(), 0, data, offset+68, 4)
        		&& !platform.getUtils().equals(KeyAgreementType.DH3K.getType(), 0, data, offset+68, 4)) {
            msgValid = false;
            logString("validateCommitMessage, Found invalid key type - "+(new String(data, offset+68, 4)));
        } else if (!platform.getUtils().equals(SasType.B32.getType(), 0, data, offset+72, 4) && !platform.getUtils().equals(SasType.B256.getType(), 0, data, offset+72, 4) ) {
            msgValid = false;
            logString("validateCommitMessage, Found invalid sas type - "+(new String(data, offset+72, 4)));
        }
        
        return (msgValid);
    }
    
    private void doDHPart1(byte[] data, int offset, int len) throws
                                               IOException, ZrtpException  {
        if (state == ZRTP_STATE_COMMIT_SENT) {
            retranTask = null;
        }
        if(msgDhPart1RX != null) {
            if(msgDhPart1RX.length != len || !platform.getUtils().equals(msgDhPart1RX, 0, data, offset, len)) {
        ...
        if (len != expectedLen) {
            logWarning("DHPart1 received with invalid length, "+len+" in mode "+dhMode);
            return;
        }
        // validate h3 from h1 (h2 in commit may not have been received, we're ignoring it if it was)
    ...
    private void getDHResult(byte[] msg) throws ZrtpException {
        if(platform.getLogger().isEnabled()) {
   		...
   		dhResult = dhSuite.getDhResult(msg, 76);
    }
    
    private void doDHPart2(byte[] data, int offset, int len) throws
                                               IOException,
                                               CryptoException {
    	...
    	if(msgDhPart2RX != null) {
            ...
            if(msgDhPart2RX.length != len || !platform.getUtils().equals(msgDhPart2RX, 0, data, offset, len)) {
                raiseDenialOfServiceWarning("DHPart2 message differs from the accepted DHPart2");
                return;
            }
            logString("Received DHPart2 - 1");
            if(platform.getLogger().isEnabled()) {
    	...
    	logString("Received DHPart2 - 2");
        if (state != ZRTP_STATE_DHPART1_SENT) {
            logString("Received unexpected DHPart2 Message, state = "+state);
            return;
        }
        logString("Received DHPart2 - 3");
        // check h2 from commit
        byte[] h1 = extractData(data, offset + 12, 32);
        logString("Received DHPart2 - 4");
        byte[] h2 = createSHA256Diggest(h1, 0, 32);
        logString("Received DHPart2 - 5");
        if (!platform.getUtils().equals(h2, 0, msgCommitRX, 12, 32)) {
        	platform.getLogger().logWarning("doDHPart2 security warning: invalid hash preimage");
            raiseDenialOfServiceWarning("DHPart2 H1 is not the preimage of the accepted Commit H2");
            return;
        }
        logString("Received DHPart2 - 6");
        int commitMacOffset = msgCommitRX.length - 8;
        byte[] commitMac = createSHA256HMAC(msgCommitRX, 0, commitMacOffset, h1);
        logString("Received DHPart2 - 7");
        if (!platform.getUtils().equals(commitMac, 0, msgCommitRX, commitMacOffset, 8)) {
            raiseDenialOfServiceWarning("Using DHPart2 H1 to hash the accepted Commit gives wrong MAC");
            return;
        }
        logString("Received DHPart2 - 8");
        // check hvi from commit
        byte[] hvi = createRemoteHvi(data, offset, len);
        logString("Received DHPart2 - 9");
        int commitHviOffset = commitMacOffset - 32; // 32 = 8 words, hvi length
        if (!platform.getUtils().equals(hvi, 0, msgCommitRX, commitHviOffset, 32)) { // 32 = 8 words, hvi length
            raiseDenialOfServiceWarning("HVI calculated from incoming DHPart2 and our Hello doesn't match Commit HVI");
            return;
        }
        logString("Received DHPart2 - 10");
        if(platform.getLogger().isEnabled()) {
        	logString("Processing DHPart2.....");
        }
        logString("Received DHPart2 - 11");
        msgDhPart2RX = extractData(data, offset, len);
        logString("Received DHPart2 - 12");
        dhResult = null;
        getDHResult(msgDhPart2RX);
        logString("Received DHPart2 - 13");
        if (dhResult == null) {
            msgDhPart2RX = null;
            return;
        }
        calculateSharedKeys();
        logString("Received DHPart2 - 14");
        sendConfirm1();
        logString("Received DHPart2 - 15");
        state = ZRTP_STATE_CONFIRM1_SENT;
    }
    
    private void doConfirm(byte[] data, int offset, int len) throws
                                                IOException,
                                                CryptoException{
        //header: 3 words
        byte[] initVector = extractData(data, offset+20, 16);   //4 words
        byte[] zrtpKey = getKeyFromKDF(s0, !initiator ? "Initiator ZRTP key" : "Responder ZRTP key", kdfContext, 256);
        //plainBytes contains H0 (8 words), flags etc. (1 word) and cache expiry (1 word)
        byte[] plainBytes = platform.getCrypto().aesDecrypt(data, offset+36, len-36, zrtpKey, initVector);
        if(platform.getLogger().isEnabled()) {
        	logBuffer("Confirm plainBytes: ", plainBytes);
        }
        ...
        // We're always sending the recommened value 0xffffffff for cache expiration interval, thus the
        // minimum of our and their interval is always their one.
        cacheExpiry = ((plainBytes[36]&0xffL)<< 24) + ((plainBytes[37]&0xffL)<<16) + ((plainBytes[38]&0xffL)<<8) + (plainBytes[39]&0xffL);
        int optlen = len - 56; // the length of the optional part should be sigLen*4, we're not checking
        logString("Confirm optlen="+optlen+";signLen="+sigLen+";d="+d+";a="+a+";v="+v+";e="+e+";cacheExpiry="+(((long)cacheExpiry)&0xffffffff));
        remoteTrust = v;
    }
    
    private void doConfirm1(byte[] data, int offset, int len) throws
                                                IOException,
                                                CryptoException {
        if (!initiator) {
        ...
        if (msgConfirm1RX != null) {
            if(msgConfirm1RX.length != len || !platform.getUtils().equals(msgConfirm1RX, 0, data, offset, len)) {
        ...
        byte[] dhPart1Mac = createSHA256HMAC(msgDhPart1RX, 0, dhPart1MacOffset, farEndH0);
        if (!platform.getUtils().equals(dhPart1Mac, 0, msgDhPart1RX, dhPart1MacOffset, 8)) {
            raiseDenialOfServiceWarning("Using Commit1 H0 to hash the accepted DHPart1 gives wrong MAC");
            return;
        }
        // check confirm_mac
        byte[] hmacKey = getKeyFromKDF(s0, "Responder HMAC key", kdfContext, dhMode.hash.getLength());
    ...
    private void doConfirm2(byte[] data, int offset, int len) throws
                                                IOException,
                                                CryptoException {
        if (initiator) {
        ...
        if (msgConfirm2RX != null) {
            if(msgConfirm2RX.length != len || !platform.getUtils().equals(msgConfirm2RX, 0, data, offset, len)) {
        ...
        byte[] dhPart2Mac = createSHA256HMAC(msgDhPart2RX, 0, dhPart2MacOffset, farEndH0);
        if (!platform.getUtils().equals(dhPart2Mac, 0, msgDhPart2RX, dhPart2MacOffset, 8)) {
            raiseDenialOfServiceWarning("Using Commit2 H0 to hash the accepted DHPart2 gives wrong MAC");
            return;
        }
        // check confirm_mac
        byte[] hmacKey = getKeyFromKDF(s0, "Initiator HMAC key", kdfContext, dhMode.hash.getLength());
        byte[] confirm_mac = createSHAHMAC(data, offset + 36, len - 36, hmacKey);
        if (!platform.getUtils().equals(confirm_mac, 0, data, offset + 12, 8)) {
        ...
        if(!delayedCacheUpdate) {
            cache.updateEntry(cacheExpiryTime(), cache.getTrust(), newRS, keepRS2, phoneNumber);
        }
    ...
    private void doError(byte[] data, int offset, int len) {
        ...
        int errCode = platform.getUtils().getInt(data, offset, offset+len); //data[offset+15] + (data[offset+14] << 8);
        logError("Received Error Code "+getZrtpErrorName(errCode));
        try {
            sendErrorACK();
        } catch (Throwable e) {
            logError("Problem occurred sending ErrorACK Message");
            // Not much we can do if here, just allow the session to be completed
        }
        sessionCompletedKO(ZrtpStrings.TEXT_ZRTP_ERROR, getZrtpErrorName(errCode));
    }
    ...
    private void doClearACK(byte[] data, int offset, int len) {
        ...
        //TODO
        /*if (iState == ZRTP_STATE_GOCLEAR_SENT) {
            iRetranTask = null;
        }*/
    }
    ...
    private void doSASrelay(byte[] data, int offset, int len) {
        ...
        //TODO
        /*if (iState == ZRTP_STATE_SASRELAY_SENT) {
            iRetranTask = null;
        }*/
    }
